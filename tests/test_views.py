...
class TestSetFlag(BaseFlagViewTest):
    ...
    def test_flagging_successfully(self):
        ...
        __, created = FlagInstance.objects.get_or_create(
            flag=flag, user=response.wsgi_request.user, reason=data["reason"]
        )
    ...
    def test_unflagging_successfully(self):
        ...
        data.pop("reason")
        response = self.client.post(self.url, data=data)
        response_data = {"status": 0, "msg": "The content has been unflagged successfully."}
        ...
        flag = Flag.objects.get_flag(post)
        __, created = FlagInstance.objects.get_or_create(
            flag=flag, user=response.wsgi_request.user, reason=FlagInstance.reason_values[0]
        )
    ...
    def test_incorrect_reason(self):
        ...
        data["reason"] = reason
        response = self.client.post(self.url, data=data)
        response_data = {"status": 1, "msg": [f"{reason} is an invalid reason"]}
    ...
    def test_choosing_last_reason_without_info(self):
        ...
        response = self.client.post(self.url, data=data)
        response_data = {"status": 1, "msg": ["Please supply some information as the reason for flagging"]}
