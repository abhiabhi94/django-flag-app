...
class FlagAPIViewsTest(BaseFlagAPITest):
    ...
    def setUp(self):
        ...
        self.url = "/api/flag/"
        self.init_count = 0
        self.context = {"model_obj": self.post, "user": self.user_1}
    ...
    def test_flagging_successfully_with_url_encoded_form(self):
        ...
        __, created = FlagInstance.objects.get_or_create(
            flag=self.flag, user=response.wsgi_request.user, reason=data["reason"]
        )
    ...
    def test_flagging_flagged_object(self):
        ...
        self.assertEqual(
            response.json()["detail"], [f"This content has already been flagged by the user ({self.user_1.username})"]
        )
    ...
    def test_flagging_unflagged_object(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(
            response.json()["detail"], [f"This content has not been flagged by the user ({self.user_1.username})"]
        )
    ...
    def test_unflagging_successfully(self):
        ...
        __, created = FlagInstance.objects.get_or_create(
            flag=self.flag, user=response.wsgi_request.user, reason=FlagInstance.reason_values[0]
        )
